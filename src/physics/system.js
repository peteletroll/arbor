...
//
  ...
  var ParticleSystem = function(repulsion, stiffness, friction, centerGravity, targetFps, dt, precision, integrator, worker){
  // also callable with ({integrator:, stiffness:, repulsion:, friction:, timestep:, fps:, dt:, gravity:, worker:})
    ...
    worker = worker === undefined ? true : !!worker;

    var _systemTimeout = (targetFps!==undefined) ? 1000/targetFps : 1000/50
    var _parameters = {integrator:integrator, repulsion:repulsion, stiffness:stiffness, friction:friction, dt:dt, gravity:centerGravity, precision:precision, timeout:_systemTimeout, worker:worker}
    var _energy
    ...
    def(Node.prototype, "p", {
      get: function() {
        ...
        def(roboPoint, "x", {
          get: function() {
            // console.log("GETTER p.x");
            return self._p.x;
          },
          set: function(newX) {
            // console.log("SETTER p.x");
            state.kernel.particleModified(self._id, { x: newX });
          }
        });
        def(roboPoint, "y", {
          get: function() {
            // console.log("GETTER p.y");
            return self._p.y;
          },
          set: function(newY) {
            // console.log("SETTER p.y");
    ...
    def(Node.prototype, "mass", {
      get: function() {
        // console.log("GETTER mass");
        return this._mass;
      },
      set: function(newM) {
        // console.log("SETTER mass", newM);
        this._mass = newM;
    ...
    def(Node.prototype, "tempMass", {
      set: function(newM) {
        // console.log("SETTER tempMass", newM);
    ...
    def(Node.prototype, "fixed", {
      get: function() {
        // console.log("GETTER fixed");
        return this._fixed;
      },
      set: function(isFixed) {
        // console.log("SETTER fixed", isFixed);
        this._fixed = isFixed;
